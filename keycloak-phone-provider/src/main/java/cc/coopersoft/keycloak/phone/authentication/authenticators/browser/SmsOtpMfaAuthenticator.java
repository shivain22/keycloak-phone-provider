...
public class SmsOtpMfaAuthenticator implements Authenticator, CredentialValidator<PhoneOtpCredentialProvider> {
  ...
  public void addCookie(AuthenticationFlowContext context, String name, String value, String path, String domain, String comment, int maxAge, boolean secure, boolean httpOnly) {
    ...
    cookieBuf.append(name).append("=").append(value);
    if (path != null) cookieBuf.append("; Path=").append(path);
    if (domain != null) cookieBuf.append("; Domain=").append(domain);
    if (maxAge >= 0) cookieBuf.append("; Max-Age=").append(maxAge);
    if (secure) cookieBuf.append("; Secure");
    if (httpOnly) cookieBuf.append("; HttpOnly");
    
    context.getSession().getContext().getHttpResponse()
        .addHeader(HttpHeaders.SET_COOKIE, cookieBuf.toString());
  }
